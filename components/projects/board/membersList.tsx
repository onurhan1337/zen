...
const MembersListDialog = ({
                               projectId, isOpen, setOpen,
                           }: MembersListDialogProps) => {
    const {data: session} = useSession();
    const {data: project} = useSWR<Project & {
        owners: User[]; members: User[]; inviteCode: string;
    ...
    return (<Dialog.Root open={isOpen} onOpenChange={(open: boolean) => setOpen(open)}>
        <Dialog.Trigger>
        ...
        >
            ...
            <Box mt={"4"}>
                <Tabs defaultValue="members">
                    <TabsList
                        className={"flex w-full max-w-screen-sm items-center justify-center"}
                    >
                        ...
                        >
                            <div className="text-sm text-zinc-400">
                                <ul className="h-[200px] w-full list-inside list-disc pr-2">
                                    {project && ((project.members && project.members.length > 0) || (project.owners && project.owners.length > 0)) ? ([...(project.members || []), ...(project.owners || []),].map((user) => (
                                        <li
...
const PendingInvites = ({
    ...
    if (!pendingInvites || pendingInvites.length === 0) {
        return (<div className="my-4 flex w-full flex-col items-center justify-center space-y-2">
    ...
    const onAcceptInvite = async ({
        ...
        code: string; userId: string; projectId: string;
    }) => {
        try {
                    ...
                    <div className="flex gap-2">
                        <Button
                            ...
                            onClick={() => onAcceptInvite({
                                code: code, userId: user.userId, projectId,
                            })}
                        >
